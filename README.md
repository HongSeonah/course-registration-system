...
기본 강의 관리 기능 외에도, 오프라인 강의 운영을 전제로 시간표 등록과 시간표 충돌 검증을 추가했습니다.
이를 바탕으로 수강 신청, 결제 확정, 수강 취소 가능 기간 제한, 대기열 승격, 강의 상태 자동 변경 기능까지 구현했습니다.
...
## AI 활용 범위
테스트 코드에 들어갈 핵심 비즈니스 로직은 제가 먼저 판단해 정리했고, AI 도구에는 그 내용을 바탕으로 테스트 구조와 작성 방향을 제안받는 데 활용했습니다. 
예를 들어 정원 초과 대기열, 대기열 승격, 취소 가능 기간 제한, 시간표 충돌 검증 같은 핵심 규칙을 우선 검증 대상으로 선택했고, AI가 제안한 Given-When-Then 구조와 Mock 기반 테스트 방식을 참고해 초안을 잡았습니다. 또한 구현 순서 정리와 문서화 초안 작성에 활용했지만, 최종 문구와 구현 내용은 직접 테스트하여 수정하고 검증했습니다.
